...
export function ActivityLog() {
  ...
  return (
    <section aria-labelledby="activity-log-title" className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-6 flex flex-col gap-5 h-full">
      <div className="flex items-center justify-between">
         <div>
           <h2 id="activity-log-title" className="text-xl font-semibold text-slate-900 tracking-tight flex items-center gap-2">
             <History aria-hidden="true" className="w-5 h-5 text-slate-500" /> Recent Actions
           </h2>
         </div>
      </div>
        ...
        <div className="flex flex-col gap-4">
          {activities.map((act, idx) => {
             ...
             return (
               ...
               >
                 <div aria-hidden="true" className={`mt-0.5 w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${styling.bg} ${styling.text}`}>
