...
export function LivingWorld() {
  ...
  return (
    <div className="w-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col relative h-64 md:h-80">
      {/* Sky */}
      <motion.div 
      ...
      >
        ...
        >
           <Sun className={`w-16 h-16 ${currentTheme.sun} fill-current transition-colors duration-1000`} />
        </motion.div>
        ...
        >
          {state === 'degraded' ? (
            <Factory className={`w-12 h-12 ${currentTheme.clouds}`} />
          ) : (
             <Cloud className={`w-16 h-16 ${currentTheme.clouds} fill-current`} />
          )}
        </motion.div>
        ...
        >
           {state === 'degraded' ? (
              <CloudRain className={`w-20 h-20 ${currentTheme.clouds} fill-current`} />
           ) : (
              <Cloud className={`w-24 h-24 ${currentTheme.clouds} fill-current`} />
           )}
        </motion.div>
      </motion.div>
      ...
      <div className="absolute w-full bottom-8 flex justify-center space-x-12 md:space-x-24 z-10">
        ...
        >
           <Trees className={`w-16 h-16 ${state === 'degraded' ? 'text-gray-800' : 'text-green-800'} transition-colors duration-1000`} />
        </motion.div>
        <motion.div
           animate={state === 'thriving' ? { rotate: [0, -2, 2, 0] } : {}}
        ...
        >
           {state === 'degraded' ? (
             <Leaf className="w-16 h-16 text-yellow-900 transition-colors duration-1000 opacity-60" />
           ) : (
             <Trees className="w-16 h-16 text-emerald-800 transition-colors duration-1000" />
           )}
        </motion.div>
         <motion.div
        ...
        >
           <Trees className={`w-12 h-12 ${state === 'degraded' ? 'text-gray-800' : 'text-green-700'} transition-colors duration-1000`} />
        </motion.div>
      </div>
